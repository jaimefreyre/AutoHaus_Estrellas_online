...
header("Access-Control-Allow-Methods: GET, POST, OPTIONS, PUT, DELETE");
    ...
    $mysql=new mysqli('localhost', 'apptablets0km', 'RenovacionPass2019', 'apptable_app');    
   ...
   while($r = $compratt->fetch_object()) {
    ...
    $b1 = $mysql->query("SELECT NOMBRE, TELEFONO, EMAIL, PATENTE, FOTO FROM EncuestaO WHERE ID LIKE $idBusqueda");
    $mysql->close();
    $b2 = array();
    /*
    while($r = $b1->fetch_object()) {
        array_push($b2, $r);
    }
    */
    
    $reponse = $b1->fetch_object();
    foreach ($reponse as $key => $val){array_push($b2, $val);}
    
    $respuesta = json_encode($b2, 128);
    echo $respuesta;
